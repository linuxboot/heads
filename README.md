...
To see a comprehensive list of all supported environment variables and options, run any wrapper script with `--help`:
...
This will display the `usage()` function from the shared `docker/common.sh` helper, documenting all environment variables (both opt-ins and opt-outs), configuration options, and their defaults. The individual wrapper scripts (`docker_latest.sh`, `docker_local_dev.sh`, `docker_repro.sh`) all source and build upon this shared helper, so the variables listed apply to all wrappers unless otherwise noted.
