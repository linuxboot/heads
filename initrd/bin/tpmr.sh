...
tpm2_unseal() {
	...
	if [ ! -f "$PRIMARY_HANDLE_FILE" ]; then
		...
		# return a distinct non-zero code so callers can present a recovery UI
		return 2
	fi
	...
	if ! tpm2 unseal -Q -c "$handle" -p "session:$POLICY_SESSION$UNSEAL_PASS_SUFFIX" \
		...
		# should succeed — return non-zero so caller can handle UI/diagnostics
		return 1
	fi
	rm -f "$TMP_ERR_FILE"
}

# Verify the TPM2 primary handle and (optionally) compare against the saved
# primary-handle hash under /boot/kexec_primhdl_hash.txt.
# Return codes:
#  0 = OK (primary handle present; matches saved hash if present)
#  2 = primary handle missing
#  3 = primary handle hash mismatch
#  4 = other verification error
tpm2_verify_primary() {
	TRACE_FUNC
	PRIMHASH_FILE="${PRIMHASH_FILE:-/boot/kexec_primhdl_hash.txt}"

	# Try to ensure we have a primary.handle file available.  If it doesn't
	# exist, attempt to read it from the persistent primary handle.
	if [ ! -f "$PRIMARY_HANDLE_FILE" ]; then
		if ! tpm2 readpublic -Q -c "$PRIMARY_HANDLE" -t "$PRIMARY_HANDLE_FILE" >/dev/null 2>&1; then
			DEBUG "tpm2_verify_primary: primary handle not present in TPM"
			# Invalidate cached owner password when the primary handle is missing
			if [ -s "$SECRET_DIR/tpm_owner_password" ]; then
				DEBUG "tpm2_verify_primary: invalidating cached TPM Owner Password because primary handle is missing"
				shred -n 3 -z -u "$SECRET_DIR/tpm_owner_password" 2>/dev/null || rm -f "$SECRET_DIR/tpm_owner_password" >/dev/null 2>&1 || true
			fi
			return 2
		fi
		DEBUG "tpm2_verify_primary: recovered primary.handle from TPM"
	fi

	# If a primhdl hash exists under /boot, compare it to the current primary
	# handle so we can detect unexpected changes (possible tampering).
	if [ -s "$PRIMHASH_FILE" ]; then
		expected_hash=$(cut -d' ' -f1 "$PRIMHASH_FILE" 2>/dev/null || true)
	t_actual=$(sha256sum "$PRIMARY_HANDLE_FILE" 2>/dev/null | cut -d' ' -f1 || true)
		if [ -n "$expected_hash" ] && [ "${t_actual:-}" != "$expected_hash" ]; then
			DEBUG "tpm2_verify_primary: primary handle hash mismatch - expected $expected_hash actual ${t_actual:-<missing>}"
			# Invalidate any cached TPM Owner Password so stale credentials aren't reused
			if [ -s "$SECRET_DIR/tpm_owner_password" ]; then
				DEBUG "tpm2_verify_primary: invalidating cached TPM Owner Password"
				shred -n 3 -z -u "$SECRET_DIR/tpm_owner_password" 2>/dev/null || rm -f "$SECRET_DIR/tpm_owner_password" >/dev/null 2>&1 || true
			fi
			return 3
		fi
