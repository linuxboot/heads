...
if [ -z "$(cat /tmp/usb_block_devices)" ]; then
	if [ -x /bin/whiptail ]; then
		...
		INPUT "USB Drive Missing! Insert your USB drive and press Enter to continue."
	fi
	sleep 1
	list_usb_storage >/tmp/usb_block_devices
	if [ -z "$(cat /tmp/usb_block_devices)" ]; then
		if [ -x /bin/whiptail ]; then
			...
			DIE "USB Drive Missing! Aborting mount."
		fi
		exit 1
	fi
fi
	...
	if [ -z ${USB_MOUNT_DEVICE} ]; then
		...
		if [ -x /bin/whiptail ]; then
			...
			while read option; do
				n=$(expr $n + 1)
				MENU_OPTIONS+=("$n" "$option")
			done </tmp/usb_disk_list

			MENU_OPTIONS+=("a" "Abort")
			whiptail_type $BG_COLOR_MAIN_MENU --title "Select your USB disk" \
			...
			STATUS "Select your USB disk:"
			n=0
			while read option; do
				n=$(expr $n + 1)
				printf '%d. %s\n' "$n" "$option" >"${HEADS_TTY:-/dev/stderr}"
			done </tmp/usb_disk_list

			INPUT "Choose your USB disk [1-$n, a to abort]:" -r option_index
		fi
