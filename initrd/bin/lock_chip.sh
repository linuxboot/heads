...
if [ -n "$APM_CNT" -a -n "$APM_STS" -a -n "$FIN_CODE" ]; then
	...
	io386 -o b -b x $APM_STS 0xff
	io386 -o b -b x $APM_CNT $FIN_CODE
	FINALIZE_STATUS=$(io386 -i b $APM_STS)
	...
	NOTE "NOT finalizing chipset - lock_chip.sh called without valid APM ports and FIN_CODE"
fi
