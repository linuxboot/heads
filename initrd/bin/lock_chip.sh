...
if [ -n "$APM_CNT" ] && [ -n "$FIN_CODE" ]; then
	...
	echo
	echo "Finalizing chipset's Write Protection through SMI PR0 lockdown call"
	io386 -o b -b x "$APM_CNT" "$FIN_CODE"
else
	echo "NOT Finalizing chipset"
	echo "lock_chip.sh called without valid APM_CNT and FIN_CODE defined under bin/lock_chip.sh."
fi
