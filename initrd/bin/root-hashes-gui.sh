...
update_root_checksums() {
	...
	whiptail_type $BG_COLOR_MAIN_MENU --title 'Root Hashes Updated and Signed' \
...
check_root_checksums() {
	...
	if ! (cd /boot && sha256sum kexec*.txt) | gpgv.sh /boot/kexec.sig - >/tmp/hash_output; then
...
while true; do
	...
	if [ -e "$HASH_FILE" ]; then
		...
		whiptail_type $BG_COLOR_MAIN_MENU --title "Root Disk Verification Menu" \
	...
	"c")
		check_root_checksums
		if [ $? -eq 0 ]; then
			whiptail_type $BG_COLOR_MAIN_MENU --title 'Verified Root Hashes' \
