...
#   Layer 1 — initramfs filesystem compat check (verifies the ISO's initrd
#             can read the USB filesystem before offering boot options)
...
# [OK] or [!] marker in /tmp/kexec_initrd_compat.txt.  The per-initrd
# flag (initrd_supports_fs) is tracked separately from the global
# "any_supported" flag so that every initrd always gets an entry —
...
# module for the USB filesystem type.  If the initrd can't read the partition
# where the ISO lives, the boot will fail after kexec.
#
...
check_initrd_fs_compat() {
	...
	local compat_file="/tmp/kexec_initrd_compat.txt"
	: > "$compat_file"
	...
	for initrd_relpath in $initrd_paths; do
		...
		# Write per-initrd marker to compat file.  boot_marker() in
		# kexec-select-boot.sh reads this per entry to show [OK]/[!]
		# before each menu line.  No entry in the compat file means
		# "cannot verify" — no marker shown, entry treated as OK.
		if [ -n "$initrd_supports_fs" ]; then
			echo "${initrd_relpath#/} $initrd_supports_fs" >> "$compat_file"
	...
	if [ -s "$compat_file" ]; then
		while IFS= read -r line; do DEBUG "Layer 1: compat: $line"; done < "$compat_file"
	fi
...
check_initrd_fs_compat "$DEV" "/boot"
...
# ISO from USB.  This approach is borrowed from u-root's iso boot support.
ADD=""
REMOVE=""
