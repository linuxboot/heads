...
read_tpm_counter() {
	...
	if [ ! -e /tmp/counter-"$counter_id" ]; then
		...
		DO_WITH_DEBUG tpmr.sh counter_read -ix "$counter_id" | tee /tmp/counter-"$counter_id" >/dev/null 2>&1 ||
...
increment_tpm_counter() {
	...
	if ! DO_WITH_DEBUG tpmr.sh counter_increment -ix "$counter_id" -pwdc '' |
		...
		if DO_WITH_DEBUG tpmr.sh counter_create -pwdc '' -la 3135106223 >/tmp/new-counter 2>/dev/null; then
