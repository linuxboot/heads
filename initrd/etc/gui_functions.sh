...
whiptail_type() {
	...
	local TYPE="$1"
	shift
	# Provide DEBUG output for troubleshooting whiptail rendering (mirrors other whiptail helpers)
	DEBUG "whiptail_type: TYPE=$TYPE"
	DEBUG "whiptail_type: BG_COLOR_MAIN_MENU=$BG_COLOR_MAIN_MENU"
	DEBUG "whiptail_type: arguments count=$#"
	local i=1
	for arg in "$@"; do
		DEBUG "whiptail_type: arg[$i]=$arg"
		i=$((i + 1))
	done
	case "$TYPE" in
		error)
