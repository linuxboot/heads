...
|---|---|---|
| `[OK]` | Green | This initramfs has the USB filesystem module (confirmed by inspecting the unpacked initrd) |
| `[!]` | Yellow | This initramfs has loadable modules but NOT the USB filesystem module — boot may fail after kexec |
| (blank) | Default | Cannot verify: initrd has no loadable modules (module may be built into kernel) |

A legend explaining these markers is shown once per session via NOTE:

```
NOTE: [OK]=compatible  [!]=may fail after kexec  (blank)=cannot verify
```
...
are written to `/tmp/kexec_initrd_compat.txt`:

```
casper/initrd [OK]
```

`boot_marker()` in `kexec-select-boot.sh` reads this file to assign the marker for
each menu entry. Initrds without an entry in the compat file get no marker (treated
as "cannot verify").
